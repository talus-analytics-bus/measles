    ...
    <div className={classNames(styles.container, styles[bubbleMetric])}>
          ...
          <div className={styles.data}>
            {tooltipItems.map(d => (
              <div className={classNames(styles[d.slug], styles.datum)}>
                ...
                <p
                  className={classNames(styles[d.slug], styles.content, {
                ...
                >
                  ...
                  {d.notAvail ? (
                    ...
                    <div className={styles.unit}>{renderUnit(d.unit)}</div>
                  )}
                  {// If value2 exists, add that
                  d.value2 !== undefined && (
                    <span className={styles.value2}>{d.value2}</span>
                  )}
                  {// If delta exists, add that
                  d.deltaData &&
                    d.deltaData.delta !== undefined &&
                    !d.notAvail && (
                      <div
                      ...
                      >
                        ...
                        <span className={styles['delta-value']}>
                          <span className={styles['num']}>
                            {d.deltaData.deltaFmt}
                          </span>
                        </span>
                        <span className={styles['delta-text']}>
                          {Util.getDeltaWord(d.deltaData.delta)} from
                          <br />
                          previous month
                        </span>
                      </div>
                ...
                {d.dataSource && !d.notAvail && (
                  <div className={classNames('dataSource', styles.dataSource)}>
                    Source: {d.dataSource}
                    {d.dataSourceLastUpdated && // TODO remove "false" when this field is ready
                      ' as of ' +
                        new Date(d.dataSourceLastUpdated).toLocaleString(
                          'en-us',
                          {
                            // TODO correctly
                            month: 'short',
      ...
      showYearlyReport && (
        <div>
          ...
          {
            <div className={styles.data}>
              {[getTooltipMetricData(popupData, 'fill')].map(d => (
                <div className={classNames(styles[d.slug], styles.datum)}>
                  <p className={classNames(styles[d.slug], styles.label)}>
                    <span>{d.label}</span>
                    <span className={'dateTimeStamp'}>
                  ...
                  <p
                    className={classNames(styles[d.slug], styles.content, {
                      [styles['notAvail']]: d.notAvail,
                      [styles.zero]:
                        d.valueNum !== undefined && d.valueNum === 0
                    })}
                  >
                    <div className={styles.value}>
                      {d.notAvail ? 'Recent data not available' : d.value}
                  ...
                  {d.dataSource && !d.notAvail && (
                    <div
                      className={classNames('dataSource', styles.dataSource)}
                    >
                      Source: {d.dataSource}
                      {d.dataSourceLastUpdated && // TODO remove "false" when this field is ready
                        ' as of ' +
