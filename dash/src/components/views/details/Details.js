...
const Details = props => {
  ...
  const [resizedSlidingLineOnce, setResizedSlidingLineOnce] = React.useState(
    false
  )
  ...
  const getWedgeChart = val => {
    function resizeWedge() {
      ...
      // TODO elegantly.
      const newWidth =
        tops[0].parentElement.parentElement.parentElement.parentElement.parentElement.getBoundingClientRect()
          .width / 6.57
    ...
    return (
      <div
        data-tip={true}
        data-for={'wedgeTooltip'}
        className={classNames(
          styles.chart,
          styles.measlesWedgeChart,
          styles.shapes
        )}
      >
  ...
  const getJeeChart = score => {
    ...
    if (score === 'legend') {
      return (
        <div className={classNames(styles.jeeChart, styles.legend)}>
      ...
      return (
        <div
          className={classNames(styles.jeeChart, styles[scoreLabeling.color])}
        >
  ...
  const getVaccActiveWedgePos = val => {
    ...
    const finalPos = targetPos >= thresh ? targetPos : 0
    return finalPos
  }
  ...
  const vaccColor = Util.getColorScaleForMetric('coverage_mcv1_infant', [
    0,
    100
  ])
  ...
  const getVaccChart = val => {
    ...
    return (
      <div
        className={classNames(styles.chart, styles.vaccChart, styles.shapes)}
      >
        <div className={classNames(styles.trapezoidContainers)}>
          {[0, 1].map(bin => (
            <div
            ...
            >
              <div
                style={{
                  ...
                  left:
                    bin === 1 && val < tooSmall
                      ? getVaccActiveWedgeWidth(val) + 5
                      : null
                }}
              >
                {bin === 1 && binData.i > 0 ? (
                  <span>
                    <span>{Util.percentize(val)}</span>
                    <span className={styles.barValUnit} />
                  </span>
                ) : (
                  ''
                )}
  ...
  const getSlidingLineJsx = () => {
    ...
    const legendEntries = noLineData
      ...
      : [
          {
            label:
              slidingLineMetric === 'incidence_monthly'
    ...
    const countSummaryJsx = countSummary !== null && (
    ...
    const resetButton = (
      <button
        ...
        className={classNames('btn-secondary btn-sm')}
        style={{
    ...
    return (
      ...
      !datum.notAvail && (
        ...
        >
          ...
          {deltaData.direction !== 'notCalc' && (
            <span className={styles['delta-text']}>
              {Util.getDeltaWord(deltaData.delta)} from
              <br />
              previous {period}
            </span>
          )}
  ...
  const getSlidingLineDataSources = () => {
    ...
    if (yDatum === undefined) ySource = null
      ...
      ySource = `Source for ${metricParams.name.toLowerCase()}: ${
        yDatum.data_source
      } as of ${yUpdated}.`
    }
    ...
    if (vaccineDatum === undefined) vaccineSource = ''
      ...
      const vaccineUpdated = new Date(vaccineDatum.updated_at).toLocaleString(
        'en-us',
        {
          month: 'short',
          year: 'numeric',
      ...
      vaccineSource = `Source for vaccination coverage: ${
        vaccineDatum.data_source
      } as of ${vaccineUpdated}.`
    }
    const sources = []
  ...
  const getCaseloadAndDeltaJsx = () => {
    const obs =
      props.countryCaseloadHistory[props.countryCaseloadHistory.length - 1]
    ...
    const caseloadAndDeltaJsx = (
      <div className={styles.value2Content}>
        ...
        caseloadAndDeltaData.value !== undefined &&
          caseloadAndDeltaData.value !== null && (
            <span>
  ...
  React.useEffect(
    function changeSlidingLineMetric() {
      if (slidingLine) {
  ...
  const getMeaslesCaseloadItems = () => {
    const sparklineBaseData = JSON.parse(
      JSON.stringify(props.countryCaseloadHistory)
    )
      .reverse()
      .slice(0, 24)
      ...
      values: [
        {
          ...
          date_time_fmt: date_time => {
            return date_time.date_time !== undefined
              ? Util.getDatetimeStamp(date_time, 'month')
              : ''
          },
          deltaData: props.countryTrendCaseload1Months
            ? Util.getDeltaData(props.countryTrendCaseload1Months)
            : undefined,
          period: 'month',
          hideSource: true,
          chart_jsx: (deltaData, i) =>
            sparklineBaseData.length > 0 ? (
              <SparkLine
        ...
        },
        {
          ...
          value_label: Util.getPeopleNoun,
          date_time_fmt: date_time => {
            return Util.getDateTimeRange({ value: date_time })
          },
          deltaData: props.countryTrendCaseload6Months
            ? Util.getDeltaData(props.countryTrendCaseload6Months)
            : undefined,
          hideSource: true,
          chart_jsx: (deltaData, i) =>
            sparklineBaseData.length > 0 ? (
          ...
          ...(props.countryCaseload6MonthsCalc
            ? props.countryCaseload6MonthsCalc
            : { value: null })
        },
        {
          ...
          deltaData: props.countryTrendCaseload6Months
            ? Util.getDeltaData(props.countryTrendCaseload12Months)
            : undefined,
          hideSource: false,
          chart_jsx: (deltaData, i) =>
            sparklineBaseData.length > 0 ? (
          ...
          ...(props.countryCaseload12MonthsCalc
            ? props.countryCaseload12MonthsCalc
            : { value: null })
        }
      ]
  ...
  const getWedgeTooltipContent = () => {
    ...
    const recentIncidence =
      props.countryIncidenceHistory[props.countryIncidenceHistory.length - 1]
    const recentCaseload =
      props.countryCaseloadHistory[props.countryCaseloadHistory.length - 1]
    ...
    if (recentCaseload === undefined) {
      ...
      return (
        <div className={styles.wedgeTooltip}>
          There were{' '}
          <span>
            {Util.comma(recentCaseload.value)} measles cases in{' '}
            {props.countryName} in{' '}
            {Util.getDatetimeStamp(recentCaseload, 'month')}
          </span>
          , or about {Util.formatIncidence(recentIncidence.value)} cases per 1M
          people. Compared to historical global trends, this is a{' '}
          <span>
            relatively {getWedgeChartLabel(binData.i)} measles incidence
          </span>
          .
        </div>
      )
      ...
      return (
        <div className={styles.wedgeTooltip}>
          There was{' '}
          <span>
            {Util.comma(recentCaseload.value)} measles case in{' '}
            {props.countryName} in{' '}
            {Util.getDatetimeStamp(recentCaseload, 'month')}
          </span>
          , or about {Util.formatIncidence(recentIncidence.value)} cases per 1M
    ...
    <div className={styles.details}>
      <div className={styles.sidebars}>
        <div className={styles.sidebar}>
          <div className={styles.title}>
            {props.countryIso2 && (
              <img
                src={`/flags/${props.countryIso2}.png`}
          ...
          {!showYearlyReport &&
            [getMeaslesCaseloadItems()].map(item => (
                  ...
                  <div className={styles.item}>
                    <span className={styles.title}>
                      ...
                      <span className={'dateTimeStamp'}>
                        {item.value !== null
                          ? `${item.date_time_fmt(item.dateTimeObs || item)}`
                          : ''}
                      </span>
                    </span>
                  ...
                  >
                    <span className={styles.title}>
                      <span className={styles.text}>
                        ...
                        item.infoTooltipText && (
                          <InfoTooltip text={item.infoTooltipText} />
                        )}
                      </span>
                          ...
                          <div className={styles.left}>
                            {
                              <span className={styles.title}>
                                ...
                                <span className={'dateTimeStamp'}>
                                  {jeeItem.date_time_fmt(
                                    jeeItem.dateTimeObs || jeeItem
                                  )}
                                </span>
                              </span>
                            }
          ...
          {[
            {
              ...
              infoTooltipText: () => (
                <span className={classNames(styles.item, styles.jee)}>
                  <span>
                    The Joint External Evaluation tool (JEE) measures
                    country-specific progress in developing the capacities
            ...
            <div className={styles.itemContainer}>
              ...
              item.type.includes('jee') && (
                <div className={classNames(styles.item, styles.jee)}>
                  <span className={styles.title}>
                    <span className={styles.text}>
                      {item.title}
                      {// If info tooltip text, render one
                      item.infoTooltipText && (
                        <InfoTooltip text={item.infoTooltipText(item)} />
                      )}
                    </span>
                    {item.legend_jsx && item.legend_jsx()}
                  </span>
      ...
      <div className={styles.main}>
        ...
        {[
          {
            title:
              slidingLineMetric === 'caseload_totalpop'
                ? 'Measles cases and vaccination over time'
                  ...
                  <div className={stylesTooltip.item}>
                    ...
                    {item.value !== null && (
                      <div className={stylesTooltip.content}>
                        <span className={stylesTooltip.value}>
                          {item.value}
                        </span>
      ...
      />
      {
        ...
        <ReactTooltip
          ...
          effect='float'
          html={true}
          getContent={tooltipData => tooltipData}
        />
      }
      {
