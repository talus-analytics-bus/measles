...
class PagingBar extends Chart {
  constructor(selector, params = {}) {
    ...
    this.setData = view => {
      ...
      const setXData = () => {
        ...
        } else if (view === 'coverage_mcv1_infant') {
          // most recent vaccinatin cov. val.
          this.data.vals.x = params.data.y2 || []
        } else {
          console.log(
            '[Error] Unrecognized view: ' + view + '. Using y series.'
          )
          this.data.vals.x = params.data.y || []
        }
      ...
      const setYData = () => {
        ...
        this.data.vals.y = this.data.vals.x
          ...
          .filter(
            v =>
              v.value !== null && !Util.yearlyReportIso2.includes(v.place_iso)
          ) // skip VE for now.
      }
      ...
      const filterData = dataTmp => {
        ...
        if (curRegion === 'all') return dataTmp
          ...
          const regionPlaces = this.params.places.find(
            d => d.name === curRegion
          ).data
      ...
      const setBarData = () => {
        ...
        this.data.vals.x.forEach(xDatum => {
          const yDatum = this.data.vals.y.find(
            y => y.place_id === xDatum.place_id
          )
          if (yDatum) {
            this.data.bars.push({
              ...yDatum,
        ...
        const pageCount =
          Math.floor(this.data.bars.length / this.params.pageLength) + 1
    ...
    const longestBarValueWidth = this.getLongestLabelWidth(
  ...
  draw() {
    ...
    const xAxisG = chart.plotAxisReact(styles, xAxis, 'x')
    ...
    const yAxisG = chart.plotAxisReact(styles, yAxis, 'y')
    ...
    xAxisLabel.append('tspan').attr('x', chart.width / 2)
    ...
    chart.update = (pageNumber, view, region = undefined) => {
      ...
      if (
        view !== chart.params.view ||
        region !== chart.params.pagingBarRegion
      ) {
        ...
        // update x scale
        const maxX =
          view === 'coverage_mcv1_infant'
            ? 100
            : d3.max(chart.data.bars, d => d.value)

        // Set domain, update axis
        x.domain([0, maxX]).nice()
        xAxis.tickFormat(chart.xMetricParams.tickFormat)
      ...
      xAxisLabel.text(
        `${chart.xMetricParams.label} (${chart.xMetricParams.dateFmt(
          chart.data.vals.x
        )})`
      )
      ...
      chart.params.setSectionTitle(
        `${chart.xMetricParams.label} by country${regionLabel}`
      )
      chart.params.setSectionDatetime(
        chart.xMetricParams.dateFmt(chart.data.vals.x)
      )
        ...
        .join(
          enter => {
            ...
            chart[styles['y-axis']]
              ...
              .attr('data-for', 'pagingBarTooltip')
              .on('click', placeName => {
                const placeDatum = chart.data.bars.find(
                  d => d.place_name === placeName
                )
                if (placeDatum) {
