...
class MiniLine extends Chart {
  constructor(selector, params = {}) {
    ...
    this.params.yMetricParams = Util.getMetricChartParams(
      this.data.vals[0].metric
    )
    ...
    if (this.data.vals.length > 0) {
      ...
      if (this.params.domain !== undefined) {
        ...
        minTime = new Date(this.data.vals[0]['date_time'].replace(/-/g, '/'))
        maxTime = new Date(
          this.data.vals[this.data.vals.length - 1]['date_time'].replace(
            /-/g,
            '/'
          )
        )
    ...
    if (chart.params.setTooltipData !== undefined) {
      ...
      chart
        ...
        .on('mousemove', function tooltipLineUpdate() {
          ...
          const item = chart.data.vals.find(d =>
            d.date_time.startsWith(xDateLineStr)
          )
          const items = []
          if (item && item.value !== null) items.push(Util.getTooltipItem(item))
