...
const Global = props => {
  ...
  const [sectionTitle, setSectionTitle] = React.useState(
    `Measles cases reported by country (${pagingBarRegion})`
  ) // PLACEHOLDER
  const [sectionDatetime, setSectionDatetime] = React.useState('') // PLACEHOLDER

  // Track which data series is being shown in the paging bar chart.
  const [pagingBarData, setPagingBarData] = React.useState(
    'cumcaseload_totalpop'
  ) // PLACEHOLDER
  ...
  const handleBackForward = change => {
    ...
    newSliderValTmp.setUTCMonth(newSliderValTmp.getUTCMonth() + change)
  ...
  const getScatterJsx = () => {
    ...
    const sliderMaxValue =
      sliderMax.getUTCFullYear() + sliderMax.getUTCMonth() / 12
    const marks = {}
    ...
    const scatterSliderValue = (
      <div className={styles.scatterSliderValue}>{curSliderValStr}</div>
    )
    const scatterArea = (
      <div className={classNames(styles.Scatter, 'Scatter-0')} />
    )
    const scatterJsx = scatterArea

    // Scatter plot legend
    const bubbleMetricData = Util.getMetricChartParams(
      props.chartParams.Scatter[0].params.data.size[0].metric
    )
    const colorMetricData = Util.getMetricChartParams(
      props.chartParams.Scatter[0].params.data.y[0].metric
    )
    const scatterLegend = (
    ...
    const pageNumbers = (
      <div className={styles.pageControls}>
        ...
        {Util.getIntArray(1, pageCount).map(i => (
          <div
            className={classNames(styles.pageControl, {
  ...
  const getScatterDataSources = () => {
    ...
    if (yDatum === undefined) yDatum = null
      ...
      ySource = `Source for ${yMetricName}: ${
        yDatum.data_source
      } as of ${yUpdated}.`
    }
    ...
    if (xDatum === undefined) xSource = ''
      ...
      xSource = `Source for vaccination coverage: ${
        xDatum.data_source
      } as of ${xUpdated}.`
    }
    const sources = []
    ...
    return [
      {
        ...
        instructions:
          'Drag slider to view data for different months. Hover on bubble to view data. Click bubble to pin country name. Double click to go to country page.',
        ...
        source_data: [
          {
            ...
            data: props.chartParams.Scatter[0].params.data.y
          },
          {
            sourceLabel: 'Source for vaccination coverage',
            data: props.chartParams.Scatter[0].params.data.x
          }
        ]
      }
    ]
    ...
    return [
      {
        ...
        source_data: [
          {
            sourceLabel: 'Source',
            data: props.chartParams.Scatter[0].params.data.size
          }
        ]
      }
    ]
    ...
    return [
      ...
      },
      {
        ...
        source_data: [
          {
            sourceLabel: 'Source',
            data: props.chartParams.MiniLine[1].params.data
          }
        ],
        infoTooltipText:
          'Simple average of vaccination coverage of infants for all reporting countries (not population-weighted)',
        date_time_fmt: date_time => {
      ...
      <div className={styles.details}>
        <div className={styles.top}>
          <div className={styles.sidebar}>
            <div className={styles.title}>Global caseload</div>
            {
              // Global minimap
        ...
        <div className={styles.bottom}>
                  ...
                  <div className={stylesTooltip.tooltipContent}>
                    ...
                    tooltipData.name !== undefined && (
                      <div className={stylesTooltip.tooltipHeader}>
                        <div
                          className={classNames(stylesTooltip.tooltipName, {
                            [stylesTooltip.placeName]:
                              tooltipData.flagPath !== undefined
                          })}
                        >
                        ...
                        <div className={stylesTooltip.content}>
                          ...
                          item.value !== null && (
                            <div>
                              ...
                              <span className={stylesTooltip.unit}>
                                {item.label}
                              </span>
                            </div>
                          )}
                          ...
                          item.value !== null &&
                            item.deltaData &&
                            item.deltaData.delta !== undefined &&
                            !item.notAvail && (
                              <div
                                className={classNames(
                                  stylesTooltip.delta,
                                  stylesTooltip[item.deltaData.direction]
                                )}
                              >
                                ...
                                <span className={stylesTooltip['delta-value']}>
                                  <span className={stylesTooltip['num']}>
                                    {item.deltaData.deltaFmt}
                                  </span>
                                </span>
                                <span className={stylesTooltip['delta-text']}>
                                  {Util.getDeltaWord(item.deltaData.delta)} from
                                  <br />
                                  previous month
                                </span>
                              </div>
          ...
          <ReactTooltip
            id={'sliderTooltip'}
            type='dark'
            className={classNames(styles.sliderTooltip, {
              visible: showSliderTooltip
            })}
            place='top'
            effect='solid'
            event='mousemove'
            offset={{ top: -window.pageYOffset }}
            getContent={() => <div>{curSliderValStr}</div>}
          />
          {
            ...
            <ReactTooltip
              ...
              place='top'
              effect='float'
              getContent={() => <div>Click to view country details page</div>}
            />
          }
        </div>
        {
